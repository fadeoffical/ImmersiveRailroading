        ...
        public void tick(Vec3i offset) {
            ...
            if (this.world.isClient) {
                if (craftingTe.getRenderTicks() % 10 == 0 && craftingTe.getCraftProgress() != 0) {
                    Audio.playSound(this.world, craftingTe.getPos(), StandardSound.BLOCK_ANVIL_PLACE, SoundCategory.BLOCKS, 1 * ConfigSound.SoundCategories.Machines.category(), 0.2f);
                }
                return;
            }
            ...
            if (progress == 0) {
                ...
                if (IRFuzzy.steelBlockOrFallback().matches(input) && output.isEmpty() && !craftingTe.getCraftItem()
                        .isEmpty()) {
