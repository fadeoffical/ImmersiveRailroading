...
public class BuilderStraight extends BuilderCubicCurve {
    ...
    public CubicCurve getCurve() {
        double length = this.info.settings.length - 1;
        CubicCurve curve = new CubicCurve(
                Vec3d.ZERO,
                VecUtil.fromYaw(length * 0.25, this.info.placementInfo.yaw),
                VecUtil.fromYaw(length * 0.75, this.info.placementInfo.yaw),
                VecUtil.fromYaw(length, this.info.placementInfo.yaw)
        );
        return curve;
    }
}
